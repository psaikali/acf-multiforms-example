...
class Shortcode
{
    ...
    public function __construct() {
        ...
        $acf_field_groups = get_posts($args);
        $arr_result = array();
        /**

         here we want to storeall the acf field groups if they start by Step to the $arr_result array.
         */
        foreach ($acf_field_groups as $field_group) {
            if (Utils::startsWith($field_group->post_title, 'Step')) {
                $field_group_name = array($field_group->post_name);
                array_push($arr_result, $field_group_name);
            }
        }

        /**
        we sort the array to have it in order...
         */
        if (isset($arr_result)) {
            # print_r($arr_result);
            sort($arr_result);
    ...
    private function requested_post_is_valid() {
        return (get_post_type((int)$_GET['post_id']) === $this->post_type && get_post_status((int)$_GET['post_id']) === 'private');
    }
    ...
    private function display_custom_message_before_form($args) {
        if ($args['post_id'] === 'new_post') {
            ...
            switch ($args['step']) {
                case 2:
                default:
                    $message = sprintf(__('Hi %1$s, il reste %2$s étapes sur %3$s thanks for your interest! Please give us some more details :)'), get_field('full_name', (int)$args['post_id']), count($this->metabox_ids)-$args['step'], count($this->metabox_ids));
                    break;

                case count($this->metabox_ids):
